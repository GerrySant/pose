...
from pose_format.utils.smplest_x import SMPLX_BODY_POINTS
...
KnownPoseFormat = Literal["holistic", "openpose", "openpose_135", "smplest-x", "alphapose_133", "alphapose_136", "sapiens", "coco_wholebody_133"]
...
def detect_known_pose_format(pose_or_header: Union[Pose,PoseHeader]) -> KnownPoseFormat:
    ...
    sapiens_components = [c.name for c in get_sapiens_components()]
...
def pose_hide_legs(pose: Pose, remove: bool = False) -> Pose:
    ...
    if known_pose_format == "holistic":
    ...
    elif known_pose_format == "smplest-x":
        words_to_look_for = [
            "L_Hip", "R_Hip",
            "L_Knee", "R_Knee",
            "L_Ankle", "R_Ankle",
        ...
        point_names_to_remove = [
            point for point in SMPLX_BODY_POINTS
            if point in words_to_look_for
        ]

        points_to_remove_dict = {
            "BODY": point_names_to_remove
        }
        ...
        point_names_to_remove = [point for point in SAPIENS_BODY_POINTS
                                 if any(word in point for word in words_to_look_for)]
        points_to_remove_dict = {"BODY_SAPIENS": point_names_to_remove}
...
def pose_shoulders(pose_header: PoseHeader) -> Tuple[Tuple[str, str], Tuple[str, str]]:
    ...
    if known_pose_format == "smplest-x":
        return ("BODY", "R_Shoulder"), ("BODY", "L_Shoulder")
    ...
    if known_pose_format == "sapiens":
        return ("BODY_SAPIENS", "right_shoulder"), ("BODY_SAPIENS", "left_shoulder")
...
def hands_indexes(pose_header: PoseHeader)-> List[int]:
    ...
    if known_pose_format == "smplest-x":
        return [
            pose_header.get_point_index("LEFT_HAND", "Middle_1"),
            pose_header.get_point_index("RIGHT_HAND", "Middle_1"),
        ]
    if known_pose_format == "alphapose_133" or known_pose_format == "alphapose_136":
        return [
...
def hands_components(pose_header: PoseHeader)-> Tuple[Tuple[str, str], Tuple[str, str, str], Tuple[str, str]]:
    ...
    if known_pose_format == "smplest-x":
        return ("LEFT_HAND", "RIGHT_HAND"), ("Wrist_Hand", "Pinky_1", "Index_1"), ("Wrist_Hand", "Middle_1")

    if known_pose_format == "alphapose_133" or known_pose_format == "alphapose_136":
        return (f"LEFT_HAND_{known_pose_format[-3:]}", f"RIGHT_HAND_{known_pose_format[-3:]}"), ("hand_0", "hand_17", "hand_5"), ("hand_0", "hand_9")

    if known_pose_format == "sapiens":
        return ("LEFT_HAND_SAPIENS", "RIGHT_HAND_SAPIENS"), ("wrist", "pinky_finger_third_joint", "forefinger_third_joint"), ("wrist", "middle_finger_third_joint")
...
def get_hand_wrist_index(pose: Pose, hand: str)-> int:
    ...
    if known_pose_format == "smplest-x":
        return pose.header.get_point_index(f"{hand.upper()}_HAND", "Wrist_Hand")
    if known_pose_format == "alphapose_133" or known_pose_format == "alphapose_136":
        return pose.header.get_point_index(f"{hand.upper()}_HAND_{known_pose_format[-3:]}", f"hand_0")
    if known_pose_format == "sapiens":
        return pose.header.get_point_index(f"{hand.upper()}_HAND_SAPIENS", "wrist")
    if known_pose_format == "coco_wholebody_133":
...
def get_body_hand_wrist_index(pose: Pose, hand: str)-> int:
    ...
    if known_pose_format == "smplest-x":
        return pose.header.get_point_index("BODY", f"{hand.upper()[0]}_Wrist")
    if known_pose_format == "alphapose_133" or known_pose_format == "alphapose_136":
